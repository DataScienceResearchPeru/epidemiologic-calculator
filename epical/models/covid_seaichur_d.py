"""Modelo SEACHUR para la propagción del Covid-19.

Parametros Epidemiologicos.
    * A1 = 0.50
        Contagio de SUSCEPTIBLE por un Infectado  [1/T]

    * A2 = 0.28
        Contagio de SUSCEPTIBLE por un Asintomatico  [1/T] ( 12.5 dias )

    * A3 = 0.35
        Periodo Latente de un Asintomatico [1/T] (5 dias)

    * A4 = 0.40
        Periodo Latente de un Infectado [1/T] (5 dias)

    * A5 = 0.20
        Periodo Latente de Asintomatico para ser un Infectado [1/T] (5 dias)

    * A6 = 0.35
        Transicion de uno en Cuarentena a Infectado [1/T]

    * A7 = 0.50
        Contagio de Susceptibles-Cuarentena por un Asintomatico [1/T]

    * A8 = 0.15
        Hospitalizado trasladado a UCI [1/T]

    * A9 = 0.30
        Transicion de un Infectado a Hospitalizado [1/T]

    * QQ = 0.0
        Stado de CUARENTENA

    * R1 = 0.20
        Recuperacion de un Asintomatico [1/T]  (30 dias)

    * R2 = 0.15
        Recuperacion de un Infectado [1/T]  (40 dias)

    * R3 = 0.07
        Recuperacion de un Hospitalizado [1/T]  (40 dias)

    * R4 = 0.03
        Recuperacion de un UCI [1/T]  (20 dias)

    * D1 = 001
        Muerte de un Infectado [1/T]

    * D2 = 002
        Muerte de un Hospitalizado [1/T]

    * D3 = 005
        Muerte de un UCI [1/T]
"""
...
EPIDEMIOLOGICAL_PARAMETERS = dict(
    A1=0.50,
    A2=0.28,
    A3=0.35,
    A4=0.40,
...
class CovidSeaichurD(Covid19Interface):
    # FIX:
    # Consider restructuration of this functions to avoid innecesary complexity
    def model(
        self, initial_conditions, duration, epidemiological_parameters=None
        ...
        # Consider restructuration of this functions to avoid innecesary complexity
        def seaichurd(
            x, t, a1, a2, a3, a4, a5, a6, a7, a8, a9, r1, r2, r3, r4, qq, d1, d2, d3
            ...
            return np.array(
                [
                    -a1 * x[0] * x[3] / population
                    - a2 * x[0] * x[2] / population
                    - qq * x[0],
                    +a1 * x[0] * x[3] / population
                    + a2 * x[0] * x[2] / population
                    - a3 * x[1]
                    - a4 * x[1]
                    + a7 * x[4] * x[2] / population,
